...
compound = "Ti"
data = load_xas_ml_data(DataQuery(compound, "FEFF")).test
spectras = data.y
features = data.X
...
pred_tuned_universal = Trained_FCModel(
    DataQuery(compound, "FEFF"), name="ft_tl"
).predictions
...
fig = plt.figure(figsize=(len(cfg.compounds) * 6, len(model_preds) * 3))
# make single plot with multiple cols for each compound
gs = fig.add_gridspec(len(model_preds), len(cfg.compounds), hspace=0, wspace=0)
axs_super = gs.subplots(sharex=True, sharey=True)
axs = axs_super.T[0]

# fig = plt.figure(figsize=(6, 8))
# gs = fig.add_gridspec(len(model_preds), 1, hspace=0, wspace=0)
...
for model, preds in model_preds.items():
    ...
    for spectra in preds:
        exafs = EXAFS(spectra, compound=compound, simulation_type="FEFF")
...
# plot the kde distribution of phaes now
fig = plt.figure(figsize=(8, 8))
gs = fig.add_gridspec(len(model_preds), 1, hspace=0, wspace=0)
axs = gs.subplots(sharex=True, sharey=True)
for ax, (model, preds) in zip(axs, model_preds.items()):
    ...
    for spectra in preds:
        ...
        phases.append(np.angle(chi_R))
        mags.append(np.abs(chi_R))
    # plot mean and std of phases
    phases = np.array(phases)
    mean_phase = np.mean(phases, axis=0)
    std_phase = np.std(phases, axis=0)
    mean_mag = np.mean(mags, axis=0)
    ax.plot(R, mean_phase, color=colors_dict[model], label=model)
    ax.plot(R, mean_mag, color=colors_dict[model], linestyle="--")
