...
def gaussAvgAndDiff(gWidth, omega, plot1, plot2):
    ...
    convPlot1 = gaussian_filter1d(
        plot1[:, 1], sigma, axis=0, order=0, output=None, mode="nearest", truncate=4.0
    )
    convPlot2 = gaussian_filter1d(
        plot2at1, sigma, order=0, output=None, mode="nearest", truncate=4.0
    )
...
def CosSimilarOld(omega, plot1, plot2, inverse=False):
    ...
    cosSimilarity = cosSimilarityA / (2.0 * norm1 * norm4) + cosSimilarityB / (
        2.0 * norm2 * norm3
    )
    if inverse:
    ...
    if plot1[-1, 1] > height1:
        ...
        if Plot:
            print(
                plot1[windowStart, 0],
                plot1[windowStop, 0],
                plot1[int(0.8 * (windowStop - windowStart)) + windowStart, 0],
            )
    ...
    if plot2[-1, 1] > height2:
        ...
        if Plot:
            print(
                plot2[windowStart, 0],
                plot2[windowStop, 0],
                plot2[int(0.8 * (windowStop - windowStart)) + windowStart, 0],
            )
        windowStop = int(0.8 * (windowStop - windowStart)) + windowStart
...
def makeInterpolateWithWindow(plot, doWindow):
    ...
    if not doWindow:
        return plot, interpolate.interp1d(
            plot[:, 0], plot[:, 1], assume_sorted=True, kind="cubic", bounds_error=False
        )
    ...
    return plot[0:windowStop, :], interpolate.interp1d(
        plot[0:windowStop, 0],
        plot[0:windowStop, 1],
        assume_sorted=True,
        kind="cubic",
...
def minCos(plot1, plot2, step=0.01):
    ...
    gplot1 = np.vstack(
        (plot1[:, 0], np.gradient(plot1[:, 1], plot1[1, 0] - plot1[0, 0]))
    ).T
    gplot2 = np.vstack(
        (plot2[:, 0], np.gradient(plot2[:, 1], plot2[1, 0] - plot2[0, 0]))
    ).T
...
def trucate_spectra(plot, erange):
    ...
    #    for i,j in enumerate(plot.ny):
    #        if j > 0.02:
    #            start = i
    #            break

    #    for i,j in enumerate (plot.nx[::-1]):
    #        if j < plot.nx[start]+35:
    #            end = len(plot.nx)-i
    #            break
    return start, end


def compare_between_codes(file1, file2, erange=35, itm=1):
    ...
    start1, end1 = trucate_spectra(file1, erange)
    plot1 = np.stack((file1.x[start1:end1] - file1.x[start1], file1.y[start1:end1])).T

    start2, end2 = trucate_spectra(file2, erange)
    plot2 = np.stack((file2.x[start2:end2] - file2.x[start2], file2.y[start2:end2])).T

    shift_prior = file1.x[start1] - file2.x[start2]

    pears, spear, coss, shift = minCos(plot1, plot2, step=0.01)

    spearman = comparePlots(shift, plot1, plot2)[
        itm
    ]  # 0 for pearson, 1 for spearman, 2 for coss
    if itm != 2:
        return shift_prior + shift, np.log10(1 - spearman)
    else:
        return shift_prior + shift, np.log10(spearman)


def truncate_spectra(spectra, erange):
    """truncate spectra from OCEAN or EXCITING

    Parameters
    ----------
    spectra: column stacked spectra, energy vs. intensity
...
def compare_between_spectra(spectra1, spectra2, erange=35, itm=1):
    """Atomatic align the spectra and calculate the spearman coefficient
    Comparison was done for epsilon instead of cross-section
    ...
    start1, end1 = truncate_spectra(spectra1, erange)
    plot1 = np.column_stack(
        (spectra1[start1:end1, 0] - spectra1[start1][0], spectra1[start1:end1, 1])
    )

    start2, end2 = truncate_spectra(spectra2, erange)
    plot2 = np.column_stack(
        (spectra2[start2:end2, 0] - spectra2[start2][0], spectra2[start2:end2, 1])
    )

    shift_prior = spectra1[start1, 0] - spectra2[start2, 0]
