...
class MLData:
    ...
    def save(compound, simulation_type):
        ...
        ml_data = MLData.prepare(compound, simulation_type)
    ...
    def prepare(compound, simulation_type):
        ...
        ids_and_sites = MLData.parse_ids_and_site(compound, data_dir)
        ml_data = p_map(
            lambda x: (
                x[0],
                x[1],
                MLData.featurize(compound, x[0], x[1]),
                *MLData.load_processed_data(compound, x[0], x[1], simulation_type).T,
            ),
            ids_and_sites[:],
        )
        return ml_data
    ...
    def featurize(compound: str, id: str, site_str: str):
        structure = MLData.get_structure(compound, id)  # reads from POSCAR
    ...
    def parse_ids_and_site(compound, dir_path):
        return [
            MLData.filename_to_uid(compound, x)
...
if __name__ == "__main__":
    ...
    for compound in compounds:
        ...
        MLData.save(compound=compound, simulation_type=simulation_type)
